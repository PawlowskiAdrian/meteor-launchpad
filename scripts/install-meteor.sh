...
if [ "$DEV_BUILD" = true ]; then
  ...
  curl -v https://install.meteor.com -o $CURRENT_DIR/install_meteor.sh
  ...
  sed -i.bak "s/RELEASE=.*/RELEASE=\"$METEOR_VERSION\"/g" $CURRENT_DIR/install_meteor.sh
  ...
  sed -i.bak "s/tar -xzf.*/bsdtar -xf \"\$TARBALL_FILE\" -C \"\$INSTALL_TMPDIR\"/g" $CURRENT_DIR/install_meteor.sh

  # install
  printf "\n[-] Installing Meteor $METEOR_VERSION...\n\n"
  sh $CURRENT_DIR/install_meteor.sh
fi
